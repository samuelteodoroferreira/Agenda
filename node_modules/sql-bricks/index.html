...
<head>
  ...
  <link rel="canonical" href="https://csnw.github.io/sql-bricks" />
...
<body>
  <div id="sidebar" class="interface">
    ...
    <ul class="toc_section">
      <li>&raquo; <a href="https://github.com/CSNW/sql-bricks">GitHub Repository</a></li>
    </ul>
  ...
  <div class="container">
    ...
    <p id="introduction">SQL Bricks was created to help eliminate DRY violations in SQL-heavy applications. SQL statements can be easily stored, cloned, modified and passed around to other parts of an application and they can generate both parameterized and non-parameterized SQL.</p>
    ...
    <p>SQL Bricks differs from similar libraries in that it does not require a schema and it is designed to be transparent, matching SQL so faithfully that developers with knowledge of SQL will pick up the API extremely quickly.</p>
    <p>The core SQL Bricks library supports the four CRUD statements (<tt>SELECT</tt>, <tt>INSERT</tt>, <tt>UPDATE</tt>, <tt>DELETE</tt>) and all of their clauses as defined by <b><a href="https://www.contrib.andrew.cmu.edu/~shadow/sql/sql1992.txt">SQL-92</a></b> (additional clauses supported by <b>Postgres</b>, <b>MySQL</b>, and <b>SQLite</b> are in the dialect-specific extension libraries, <a href="https://github.com/Suor/sql-bricks-postgres">sql-bricks-postgres</a>, <a href="https://github.com/tamarzil/mysql-bricks">mysql-bricks</a> and <a href="https://github.com/CSNW/sql-bricks-sqlite">sql-bricks-sqlite</a>, respectively). Adding support for other SQL statements (<tt>CREATE</tt>, <tt>ALTER TABLE</tt>, etc) would clutter the library without providing much real benefit.</p>
    <p>The <a href="https://github.com/CSNW/sql-bricks">source</a> is on GitHub and over <a href="browser-tests.html">200 tests</a> are available for your perusal.</p>

    <h2>Related Libraries</h2>
    <p><a href="https://github.com/tamarzil/mysql-bricks">mysql-bricks</a> adds mysql-dialect extensions, like <tt>INSERT ... ON DUPLICATE KEY UPDATE ...</tt>, <tt>INSERT IGNORE ...</tt>, <tt>LIMIT (SELECT / UPDATE / DELETE)</tt>, <tt>OFFSET</tt>, <tt>ORDER BY (UPDATE / DELETE)</tt></p>
    ...
    <div id="documentation">
      <h2 id="use">Use</h2>
      <p>
        <p>In <a href="https://nodejs.org">node</a>, SQL Bricks can be accessed with <tt>require('sql-bricks')</tt>. In the browser, it can be accessed with the <tt>SqlBricks</tt> global. SQL Bricks has no production dependencies; its only dev dependency is Mocha.js. Throughout this documentation, it is assumed that SQL Bricks is set to a local <tt>sql</tt> variable and that top-level functions are also set to local variables (for example: <tt>var sql = SqlBricks, select = sql.select, or = sql.or, $in = sql.in;</tt>).</p>
...
// SELECT * FROM person WHERE billing_addr_id = mailing_addr_id
      ...
      <p>Multiple <b>values</b> can be passed in as separate arguments or as an array (this example uses the postgres <a href="https://www.postgresql.org/docs/9.4/static/functions-json.html#FUNCTIONS-JSONB-OP-TABLE">jsonb contains operator</a>, <tt>@&gt;</tt>):
        <pre>
...
// SELECT * FROM person WHERE active = TRUE AND local = TRUE
      ...
      <p id="toString">
        ...
        <br />
        While <tt>toString()</tt> is easy to use, it is not recommended in most cases because:
        <ul><li>It doesn't provide <a href="https://en.wikipedia.org/wiki/SQL_injection#Parameterized_statements">robust protection</a> against SQL injection attacks (it just does basic escaping)</li>
        <li>It doesn't provide as much support for complex data types (objects, arrays, etc, are "stringified" before being passed to your database driver, which then has to interpret them correctly)</li>
        <li>It does not provide the same level of detail in error messages (see <a href="https://github.com/Suor/sql-bricks-postgres/issues/10">this issue</a>)</li></ul>
        For the above reasons, it is usually better to use <tt>toParams()</tt>.
      </p>
...
// SELECT * FROM person NATURAL INNER JOIN address
      ...
      <p id="selwhere">
        ...
        <code>sel.where(whereExpr)</code><br />
        <code>sel.where(sql)</code>
...
// SELECT * FROM person WHERE last_name LIKE 'Flint%' OR first_name = 'Fred'
        ...
        <p>If necessary, literal SQL can be passed in via <a href="#sql"><tt>sql(str[, values...])</tt></a>:</p>
        <pre>
...
// SELECT person."order" AS person_order FROM person
      ...
      <p>Thanks to <a href="https://github.com/Suor">Suor</a> for taking over maintenance of the postgres-dialect extension library: <a href="https://github.com/Suor/sql-bricks-postgres">sql-bricks-postgres</a>. Thanks to <a href="https://github.com/tamarzil">tamarzil</a> for creating and maintaining the mysql-dialect extension library: <a href="https://github.com/tamarzil/mysql-bricks">mysql-bricks</a></p>
      ...
      <p>Also, thanks to <a href="https://github.com/jashkenas">jashkenas</a> for Underscore.js and its documentation, which served as a model and template for these docs.</p>
